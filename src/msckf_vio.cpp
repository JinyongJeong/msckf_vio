...
bool MsckfVio::loadParameters()
{
  ...
  ROS_INFO("initial velocity: %f, %f, %f", state_server.imu_state.velocity(0), state_server.imu_state.velocity(1),
...
void MsckfVio::featureCallback(const CameraMeasurementConstPtr& msg)
{
  ...
  if (processing_time > 1.0 / frame_rate)
  {
    ...
    printf("Remove lost features time: %f/%f\n", remove_lost_features_time,
           remove_lost_features_time / processing_time);
...
void MsckfVio::mocapOdomCallback(const nav_msgs::OdometryConstPtr& msg)
{
  ...
  if (publish_tf)
  {
    ...
    tf_pub.sendTransform(tf::StampedTransform(T_b_w_gt_tf, msg->header.stamp, fixed_frame_id, child_frame_id + "_moca"
                                                                                                               "p"));
  }
...
void MsckfVio::processModel(const double& time, const Vector3d& m_gyro, const Vector3d& m_acc)
{
  ...
  Vector3d w2 =
      skewSymmetric(dtime * imu_state.velocity_null + imu_state.position_null - imu_state.position) * IMUState::gravity;
...
void MsckfVio::predictNewState(const double& dt, const Vector3d& gyro, const Vector3d& acc)
{
  ...
  if (gyro_norm > 1e-5)
  {
    ...
    dq_dt2 =
        (cos(gyro_norm * dt * 0.25) * Matrix4d::Identity() + 1 / gyro_norm * sin(gyro_norm * dt * 0.25) * Omega) * q;
  }
  else
  {
...
void MsckfVio::measurementUpdate(const MatrixXd& H, const VectorXd& r)
{
  ...
  MatrixXd S =
      H_thin * P * H_thin.transpose() + Feature::observation_noise * MatrixXd::Identity(H_thin.rows(), H_thin.rows());
...
void MsckfVio::pruneCamStateBuffer()
{
  ...
  for (const auto& cam_id : rm_cam_state_ids)
  {
    ...
    {
      state_server.state_cov.block(cam_state_start, 0, state_server.state_cov.rows() - cam_state_end,
                                   state_server.state_cov.cols()) =
          state_server.state_cov.block(cam_state_end, 0, state_server.state_cov.rows() - cam_state_end,
                                       state_server.state_cov.cols());

      state_server.state_cov.block(0, cam_state_start, state_server.state_cov.rows(),
                                   state_server.state_cov.cols() - cam_state_end) =
          state_server.state_cov.block(0, cam_state_end, state_server.state_cov.rows(),
                                       state_server.state_cov.cols() - cam_state_end);
...
void MsckfVio::onlineReset()
{
  ...
  if (position_x_std < position_std_threshold && position_y_std < position_std_threshold &&
